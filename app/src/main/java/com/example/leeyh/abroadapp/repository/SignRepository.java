...
public class SignRepository {
    ...
    private void storageProfileToServer(byte[] bitmapArray) {
    ...
    private void saveUserInfoToServerDB(Uri profileUri) {
        FirebaseUser user = mAuth.getCurrentUser();
        String uid = user.getUid();
        String email = user.getEmail();
        UserModel myInfo;
        if (profileUri != null) {
            myInfo = new UserModel(uid, email, profileUri.toString());
        } else {
            myInfo = new UserModel(uid, email, null);
        }
        mDatabaseRef.child(uid).setValue(myInfo).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                mListener.onTaskFinished(ERROR);
            }
    ...
    public void createUser(String email, String password, final byte[] profileByteArray) {
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    //storage user profile
                    storageProfileToServer(profileByteArray);
                }
            }
